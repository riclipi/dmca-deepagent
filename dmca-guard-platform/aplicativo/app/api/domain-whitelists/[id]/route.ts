  ...
  try {
    ...
    if (whitelistEntry.userId !== session.user.id) {
      ...
      await createAuditLog(
        session.user.id,
        'domain_whitelist_delete_attempt_forbidden',
        'DomainWhitelist',
        { targetUserId: whitelistEntry.userId, domain: whitelistEntry.domain, reason: "Forbidden access" }
      );
